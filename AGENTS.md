...
## Что сделать завтра (17.09.2026)
1. **vc.ru — ОТЛОЖЕНО ДО 01.10** (по решению пользователя «с первого числа»): (а) проверить в 3 опубликованных статьях, что ссылки uriy-as.org реально кликабельны (при Ctrl+Shift+V markdown-ссылки теряют гиперссылку — это причина 0 кликов); (б) опубликовать #4 `vcru-website-care.md` (уже содержит ссылки uriy-as.org + t.me)
2. **Medium**: подача в публикации (Better Programming, Towards Dev, Plain English) — выбрать 1 статью (кандидат «10 Website Mistakes», EN) и подать. Заполнить профиль: аватар + bio (по `medium-profile-guide.md`)
3. **Криптокошелёк для бизнеса**: требуется принимающий+выводящий вариант. Выбран кандидат MEXC (рег. открылась случайно, работает из Украины, zero-fee-акция; обязателен KYC до крупных сумм, бэкап, не хранить крупное). Альтернативы: Binance (риск рестрикций RU-стран), Trust Wallet (только приём, вывод через биржу). Кнопки Plisio на сайте НЕ делать до готовности кошелька
4. **Новый сайт для увеличения клиентской базы**: пересмотреть после паузы Хабра — Dev.to (разгон), vc.ru (01.10 пост готов), каталоги (Google Business Profile/2GIS/Clutch)
5. **Habr**: ПАУЗА (решение принято) — не публиковать, пока не будут готовы писать текст руками без ИИ
