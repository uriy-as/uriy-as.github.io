...
## Что сделать завтра (18.09.2026)
1. **vc.ru**: НЕ спешить с выводами по кликам — гиперссылки добавлены 16-17.09, аналитика (период 17.08-16.09: 50 открытий, 0 кликов) пока не учитывает их. Перепроверить 19-20.09: если 0 кликов к 20.09 — ссылки не спаслись или аудитория не дочитала до подвала
2. **Google / GSC**: привязан ли `sitemap.xml` → «Индексирование → Страницы» (55 URL) → «Запросить индексацию». Цифры в чат
3. **Medium**: подача «10 Website Mistakes» (EN) в публикации + профиль (аватар, bio по `medium-profile-guide.md`)
4. **Криптокошелёк**: MEXC (KYC, бэкап). Кнопки Plisio НЕ делать
5. **Новый сайт для клиентской базы**: Dev.to (разгон), каталоги
6. Проверить посещения сайта (API), Dev.to
