...
1. **Google / GSC (добавлено по просьбе пользователя)**: проверить вручную — привязан ли `sitemap.xml` в GSC → «Индексирование → Страницы» (сравнить с 55 URL) → «Запросить индексацию» для непроиндексированных. Цифры вставить в чат
2. **vc.ru**: посмотреть в аналитике кабинета, пошли ли клики по гиперссылкам (добавлены 16-17.09). Остальное — 01.10 (проверка ссылок в оставшихся постах, публикация #4 `vcru-website-care.md`)
...
4. **Криптокошелёк для бизнеса**: принимающий+выводящий — MEXC (KYC до крупных сумм, бэкап). Кнопки Plisio НЕ делать до готовности кошелька
5. **Новый сайт для клиентской базы**: пересмотреть — Dev.to (разгон), vc.ru (01.10), каталоги (Google Business Profile/2GIS/Clutch)
6. **Habr**: ПАУЗА (текст руками без ИИ, когда будет готовность)
7. Проверить посещения сайта (API), Dev.to
